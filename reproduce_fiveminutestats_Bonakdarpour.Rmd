...
**NOTE:** The following is taken directly from the fiveMinuteStats page...
I found it to be a more straightforward introduction to the EM alg. use than most other places.

# Overview

Recall that if our observations $X_i$ come from a mixture model with $K$ mixture components, the marginal probability distribution of $X_i$ is of the form:
...
\[
\ell(\theta)=\sum_{i=1}^{n} \log \left(\sum_{k=1}^{K} \pi_{k} \underbrace{N\left(x_{i} ; \mu_{k}, \sigma_{k}^{2}\right)}_{L[i, k]}\right)
\]
